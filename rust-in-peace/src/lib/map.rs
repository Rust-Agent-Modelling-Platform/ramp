...
impl MapInstance {
    ...
    pub fn set(&mut self, island_env: &mut IslandEnv, x: i64, y: i64, val: i32) {
        ...
        let range = self.map.owners.keys().find(|&r| r.contains(&offset.try_into().unwrap())).unwrap();
    ...
    pub fn get_value(&self, pos: (i64, i64)) -> i32 {
        ...
        if range_start == 0 {
            ...
            local_index = offset - (range_start as i64 / scale) * scale;
        }
        if self.data.get(local_index as usize) == None {
            -1
        } else {
            return *self.data.get(local_index as usize).unwrap();
        }
    }
